...
par(mar=c(4.1,5.7,1,1)+0.1)

time <-seq(1:nrow(your_data_variable_T))
cols <- rainbow(ncol(your_data_variable_T))
# Plot the first variable
plot(time, your_data_variable_T[, 1], type = "l", col = cols[1], las=0, cex.axis=1.75, cex.lab=1.65, xlab = "", ylab = "Lotka-Volterra model \n (Transcritical)",
     ylim = c(0,1.3))
...
points(c(6700,6700),c(-100,200), type="l", col="red", lwd=1.5, lty=2)
mtext("A",side=3, at=0, line=0.5,cex=1.5)

#### plot dev

par(mar=c(4.1,4.3,1,1)+0.1)

plot(0,0,xlim=c(0,6800), ylim=c(0.63,1.05), type="p", col="white", las=0, cex.axis=1.75, cex.lab=1.5, main="", xlab="", ylab="")
...
cols <- rep("lightblue", n)  # start all as lightblue
# For the last 50 points, create a gradient from lightblue to blue
grad_cols <- colorRampPalette(c("lightblue", "blue"))(150)
# Assign gradient to last 50 points
cols[(n-150+1):n] <- grad_cols

points(multi_result[,1], multi_result[,2], type="p", cex=1.5, pch=20, col=cols)
...
# smoothed_data <- savgol(multi_result[,2], 51)
# lines(multi_result[10:260,1],smoothed_data[10:260], col = 2,lwd=2)

# Fit a smooth spline
fit <- smooth.spline(multi_result[,1], multi_result[,2], spar = 0.6) # adjust spar for smoothness
# Predict only in your desired range
smoothed_data <- predict(fit, multi_result[,1])
# Add to plot
lines(smoothed_data, col=2, lwd=2)

mtext("B",side=3, at=0, line=0.5,cex=1.5)

## plot dev in complex

par(mar=c(4.1,1.5,1,4.5)+0.1)

f <- function(x) exp(-(0+1i)*x)
x <- seq(0, 2*pi, by=0.01)

plot(f(x), cex.lab=1.65, cex.axis=1.75, type="l", xlab="", ylab="", xlim=c(0.6,1), ylim=c(-0.2,0.2), yaxt="n")
...
emp_time_series <- scale(mat_Isomap_S[1:9000,2:4])
...
# Plot the first variable
plot(time, your_data_variable_S[, 1], type = "l", col = cols[1], las=0, cex.axis=1.75, cex.lab=1.65, xlab = "", ylab = "Lotka-Volterra model \n (Saddle-node)",
     ylim = c(0,1.3))
...
plot(0,0,xlim=c(0,9000), ylim=c(0.63,1.05), type="p", col="white", las=0, cex.axis=1.75, cex.lab=1.5, main="", xlab="", ylab="|MM-DEV|")
points(c(0,125000),c(1,1), type="l", col="gray", lwd=3, lty=2)
...
# smoothed_data <- savgol(multi_result[,2], 51)
# lines(multi_result[10:350,1],smoothed_data[10:350], col = 2,lwd=2)
...
plot(f(x), cex.lab=1.65, cex.axis=1.75, type="l", xlab="", ylab="", xlim=c(0.6,1.05), ylim=c(-0.2,0.2), yaxt="n")

#n <- NROW(multi_result)

points(c(-1,1), c(0,0), lty=1, lwd=1.5, type="l", col="black")
points(multi_result[(n-150+1):n,3], multi_result[(n-150+1):n,4], lty=1, lwd=1.5, type="p", cex=2.75, pch=20, col=cols[(n-150+1):n])

#2nd y-axis
axis(4, las=0, col="black", cex.axis=1.65)
mtext("Im(MM-DEV)",side=4,line=3,cex=1.0)
...
# # for time series with 1000 points only
kk1=read.csv('Ricker21_1000.csv',header=T)
...
cols <- rainbow(21)
# Plot the first variable
plot(time, kk1[, 2], type = "l", col = cols[1], las=0, cex.axis=1.75, cex.lab=1.65, xlab = "Time", ylab = "Ricker model \n (Period doubling)",
     ylim = c(0,1.5))

# Add the other lines
for (i in 3:6) {
  lines(time, kk1[, i], col = cols[i])
}
...
plot(0,0,xlim=c(0,500), ylim=c(0.3,1.02), type="p", col="white", las=0, cex.axis=1.75, cex.lab=1.5, main="", xlab="Time", ylab="")
...
plot(f(x), cex.lab=1.65, cex.axis=1.75, type="l", xlab="Re(MM-DEV)", ylab="", xlim=c(-1,0.5), ylim=c(-0.2,0.2), yaxt="n")
